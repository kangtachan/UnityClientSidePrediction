...
public class Client : MonoBehaviour
{
    ...
    [SerializeField] PaddleController m_paddle = null;
    ...
    {
        ClientState state;
        state.rigidbody_states = new RigidbodyStateNoVel[m_syncedRigibodies.Length];
        for (int i = 0; i < m_syncedRigibodies.Length; i++)
        {
            state.rigidbody_states[i].position = m_syncedRigibodies[i].position;
            state.rigidbody_states[i].rotation = m_syncedRigibodies[i].rotation;
        }
